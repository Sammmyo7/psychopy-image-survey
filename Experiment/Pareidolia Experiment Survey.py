...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    for thisComponent in Experiment1_countdownComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine:
        ...
        # if OnionGrid is starting this frame...
        if OnionGrid.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            OnionGrid.frameNStart = frameN  # exact frame index
            OnionGrid.tStart = t  # local t and not account for scr refresh
            OnionGrid.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(OnionGrid, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'OnionGrid.started')
            # update status
            OnionGrid.status = STARTED
            OnionGrid.setAutoDraw(True)
        
        # if OnionGrid is active this frame...
        if OnionGrid.status == STARTED:
            # update params
            pass
    ...
    for thisComponent in Experiment2_imageselectionComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine:
        ...
        if TreeGrid.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            TreeGrid.frameNStart = frameN  # exact frame index
            TreeGrid.tStart = t  # local t and not account for scr refresh
            TreeGrid.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(TreeGrid, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'TreeGrid.started')
            # update status
            TreeGrid.status = STARTED
            TreeGrid.setAutoDraw(True)
        
        # if TreeGrid is active this frame...
        if TreeGrid.status == STARTED:
            # update params
            pass
